<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_model.aspx.cs" Inherits="sparepartproject.systemconfig.spi_model" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Model</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        <li>
                            <a href="spi_model.aspx" style="background-color: white; color: #172679;">Model</a>
                        </li>
                        <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Model</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color:#f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color:#003366;">Model</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                    ...
                    <button id="btnAddModel" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                        <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Model
                    </button>
                </div>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 id="addModelModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Model</h5>
                            ...
                            <div class="modal-body">
                                <div style="padding-left: 20px; padding-right: 20px;">
                                    <div class="form-group mt-3 mb-5">
                                        <label for="modelName" style="font-weight: 500; font-size: 1rem;">New Model Name</label>
                                        <input type="text" class="form-control" id="modelName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" style="font-size: .8rem;" data-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Model</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        $('#btnAddModelToDB').click(function () {
            var modelName = $('#modelName').val().toUpperCase(); // Ubah menjadi uppercase
            if (modelName) {
                // Menampilkan SweetAlert untuk konfirmasi
                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: "Model " + modelName + " akan ditambahkan.",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/api/models/check',  // URL endpoint untuk cek model
                            type: 'POST',
                            data: JSON.stringify({ modelName: modelName }),  // Kirim data model
                            contentType: 'application/json',
                            success: function (response) {
                                if (response.exists) {
                                    Swal.fire({
                                        title: "Double!",
                                        text: "Model tersebut sudah ada di Database?",
                                        icon: "warning"
                                    });
                                } else {
                                    // Jika tidak ada, simpan ke database
                                    $.ajax({
                                        url: '/api/models/add',  // URL endpoint untuk menambah model
                                        type: 'POST',
                                        data: JSON.stringify({ modelName: modelName }),
                                        contentType: 'application/json',
                                        success: function (result) {
                                            if (result.success) {
                                                Swal.fire('Berhasil', 'Model berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                                $('#addModelModal').modal('hide');  // Menutup modal
                                                // Reset nilai input
                                                $('#modelName').val('');
                                                $('#searchInput').val('');

                                                loadModels(currentPage);  // Muat ulang data model
                                            } else {
                                                Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan model.', 'error');
                                            }
                                        },
                            ...
                            error: function (error) {
                                notyf.error('Error saat memeriksa Model Name yang sudah ada di Database!');
                            }
                        });
                    }
                });
            } else {
                notyf.error('New Model Name tidak boleh kosong!');
            }
        });
        ...
        function loadModels(page, search = "") {
            $.ajax({
                url: '/api/models?page=' + page + '&pageSize=' + pageSize + '&search=' + encodeURIComponent(search),
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    if (response.data && response.data.length > 0) {
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-outline-danger btn-sm delete-model" data-id="${model.Id}">
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadModels(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
        ...
        $('#searchInput').on('input', function () {
            ...
            loadModels(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });
        ...
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadModels(currentPage); // Muat data untuk halaman 1 pada awalnya
        });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            var modelId = $(this).data('id');  // Mendapatkan ID model dari atribut data-id
            var modelName = $(this).closest('tr').find('td:nth-child(2)').text();  // Mendapatkan nama model dari atribut data-name

            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Model berikut : ' + modelName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/models/delete/' + modelId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Model sudah di delete, lihat data terbaru pada table.', 'success');
                                loadModels(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');

                            } else {
                                Swal.fire('Error!', 'Gagal delete model.', 'error');
                            }
                        },
                        error: function (xhr, status, error) {
                            Swal.fire('Error!', 'Error ketika delete model ke Database.', 'error');
                        }
                    });
                }
            });
