<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_employee.aspx.cs" Inherits="sparepartproject.systemconfig.spi_employee" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Employee</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        ...
                        <li>
                            <a href="spi_employee.aspx" style="background-color: white; color: #172679;">Employee</a>
                        </li>
                        <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Employee</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color:#f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color:#003366;">Employee</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                            <div class="search-container">
                                <input type="text" id="searchInput" style="font-size: .7rem;" placeholder="Search..." />
                                <button id="btnAddEmployee" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                                    <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Employee
                                </button>
                            </div>
                            <table class="table table-hover" id="employeesTable" style="font-size: .8rem; cursor: pointer;">
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 id="addEmployeeModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Employee</h5>
                                        ...
                                        <div class="modal-body">
                                            <div style="padding-left: 20px; padding-right: 20px;">
                                                <div class="form-group mt-3 mb-4">
                                                    <label for="vendorName" style="font-weight: 500; font-size: 1rem;">New Employee ID</label>
                                                    <input type="text" class="form-control" id="employeeID" style="font-size: .8rem; width: 100%; text-transform: uppercase;" placeholder="" />
                                                </div>
                                                <div class="form-group mt-3 mb-4">
                                                    <label for="vendorName" style="font-weight: 500; font-size: 1rem;">New Employee Name</label>
                                                    <input type="text" class="form-control" id="employeeName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                                </div>
                                                <div class="form-group mt-3 mb-4">
                                                    <label for="vendorName" style="font-weight: 500; font-size: 1rem;">New Employee Email</label>
                                                    <input type="text" class="form-control" id="employeeSection" style="font-size: .8rem; width: 100%;" />
                                                </div>
                                            </div>
                                        </div>
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" style="font-size: .8rem;" data-dismiss="modal">Close</button>
                                            <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Employee</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $('#btnAddEmployee').click(function (e) {
            ...
            $('#addEmployeeModal').modal('show');  // Menampilkan modal
        });

        let currentPage = 1; // Halaman saat ini
        const pageSize = 10; // Jumlah item per halaman

        function loadEmployee(page, search = "") {
            $.ajax({
                url: '/api/employee?page=' + page + '&pageSize=' + pageSize + '&search=' + encodeURIComponent(search),
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    if (response.data && response.data.length > 0) {
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-outline-danger btn-sm delete-model" data-id="${unit.EmpNo}">
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadEmployee(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
                        ...
                        $('#employeesTable tbody').append('<tr><td colspan="3" style="text-align:center; color:red;">Data tidak ditemukan</td></tr>');
                    }
                },
        ...
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadEmployee(currentPage); // Muat data untuk halaman 1 pada awalnya
        });
        ...
        $('#searchInput').on('input', function () {
            ...
            loadEmployee(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });

        $('#btnAddModelToDB').click(function () {
            //var empName = $('#').val().toUpperCase(); // Ubah menjadi uppercase
            //var empNo = $('#').val().toUpperCase(); // Ubah menjadi uppercase
            //var empEmail = $('#').val().toUpperCase(); // Ubah menjadi uppercase

            var employeeReq = {
                EmpName: $('#employeeName').val().toUpperCase(),
                EmpID: $('#employeeID').val().toUpperCase(),
                EmailAddress: $('#employeeSection').val()
            ...
            if (employeeReq) {
                ...
                Swal.fire({
                    ...
                    text: "Employee " + employeeReq.EmpName + " akan ditambahkan.",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/api/employee/check',  // URL endpoint untuk cek model
                            type: 'POST',
                            data: JSON.stringify(employeeReq),  // Kirim data model
                            contentType: 'application/json',
                            success: function (response) {
                                if (response.exists) {
                                    Swal.fire({
                                        title: "Double!",
                                        text: "Employee "+employeeReq.EmpID+" sudah ada di Database.",
                                        icon: "warning"
                                    });
                                } else {
                                    // Jika tidak ada, simpan ke database
                                    $.ajax({
                                        url: '/api/employee/add',  // URL endpoint untuk menambah model
                                        type: 'POST',
                                        data: JSON.stringify(employeeReq),
                                        contentType: 'application/json',
                                        success: function (result) {
                                            if (result.success) {
                                                Swal.fire('Berhasil', 'Employee berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                                $('#addEmployeeModal').modal('hide');  // Menutup modal
                                                // Reset nilai input
                                                $('#employeeName').val('');
                                                $('#employeeSection').val('');
                                                $('#employeeID').val('');
                                                ...
                                                loadEmployee(currentPage);  // Muat ulang data model
                                            } else {
                                                Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan Employee.', 'error');
                                            }
                                        },
                ...
                notyf.error('New Employee Name tidak boleh kosong!');
            }
        });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            var empId = $(this).data('id');  // Mendapatkan ID model dari atribut data-id
            var empName = $(this).closest('tr').find('td:nth-child(2)').text();  // Mendapatkan nama model dari atribut data-name

            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Employee berikut : ' + empName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/employee/delete/' + empId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Employee sudah di delete, lihat data terbaru pada table.', 'success');
                                loadEmployee(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');

                            } else {
                                Swal.fire('Error!', 'Gagal delete Employee.', 'error');
                            }
                        },
                        error: function (xhr, status, error) {
