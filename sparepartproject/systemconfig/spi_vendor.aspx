<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_vendor.aspx.cs" Inherits="sparepartproject.systemconfig.spi_vendor" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Vendor</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        ...
                        <li>
                            <a href="spi_vendor.aspx" style="background-color: white; color: #172679;">Vendor</a>
                        </li>
                        <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Vendor</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color: #f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color: #003366;">Vendor</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                            <div class="search-container">
                                ...
                                <button id="btnAddModel" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                                    <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Vendor
                                </button>
                            </div>
                            <table class="table table-hover" id="vendorsTable" style="font-size: .8rem; cursor: pointer;">
                                ...
                                <div class="modal-dialog modal-md">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 id="addModelModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Vendor</h5>
                                        ...
                                        <div class="modal-body">
                                            <div style="padding-left: 20px; padding-right: 20px;">
                                                <div class="form-group mt-3 mb-4">
                                                    <label for="vendorName" style="font-weight: 500; font-size: 1rem;">New Vendor Name</label>
                                                    <input type="text" class="form-control" id="vendorName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                                </div>
                                                <div class="form-group mt-1 mb-5">
                                                    <label for="suppName" style="font-weight: 500;">New Vendor Location</label>
                                                    <select class="form-control" id="selectLocation" required>
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Vendor</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadVendors(currentPage); // Muat data untuk halaman 1 pada awalnya
            ...
            $.ajax({
                type: "POST",
                url: "spi_vendor.aspx/GetLocations", // Gantilah dengan nama file dan method backend
                data: '{}',
                ...
                success: function (response) {
                    ...
                    var unitsSelect = $('#selectLocation');

                    // Kosongkan dropdown terlebih dahulu
                    unitsSelect.empty();
                    unitsSelect.append('<option value="">-- Pilih Location --</option>');
        ...
        $('#btnAddModelToDB').click(function () {
            var vendorRequest = {
                VendorName: $('#vendorName').val().toUpperCase(),
                Location: $('#selectLocation').val().toUpperCase()
            };

            console.log(vendorRequest);

            if (!vendorRequest.VendorName || !vendorRequest.Location) {
                notyf.error('New Vendor Name dan Location tidak boleh kosong!');
            ...
            Swal.fire({
                ...
                text: "Vendor " + vendorRequest.VendorName + " akan ditambahkan.",
                icon: 'warning',
                ...
                customClass: {
                    confirmButton: 'btn-primary-swal',
                }
            }).then((result) => {
                if (result.isConfirmed) {
                    // Memeriksa keberadaan vendor
                    $.ajax({
                        url: '/api/vendor/check',
                        type: 'POST',
                        data: JSON.stringify(vendorRequest), // Kirim data model
                        contentType: 'application/json',
                        success: function (response) {
                            if (response.exists) {
                                //notyf.error('Vendor ' + vendorRequest.VendorName + ' sudah ada di database!'); 
                                //Swal.fire('Vendor ' + vendorRequest.VendorName + ' sudah ada di database!', 'error');
                                Swal.fire({
                                    title: "Double!",
                                    text: "Vendor tersebut sudah ada di database!",
                                    icon: "error"
                                });
                            } else {
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        if (result.success) {
                                            Swal.fire('Berhasil', 'Vendor berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                            ...
                                            Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan Vendor.', 'error');
                                        }
                                    },
                        ...
                        error: function (error) {
                            notyf.error('Error saat memeriksa Vendor Name yang sudah ada di Database!');
                        }
                    });
                }
            });
        ...
        function loadVendors(page, search = "") {
            $.ajax({
                url: '/api/vendor?page=' + page + '&pageSize=' + pageSize + '&searchTerm=' + encodeURIComponent(search), // Perbaikan di sini
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    if (response.data && response.data.length > 0) {
                        ...
                        <tr>
                            ...
                            <td>
                                <button class="btn btn-outline-danger btn-sm delete-model" data-id="${vendor.Id}">
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadVendors(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
                        ...
                        $('#vendorsTable tbody').append('<tr><td colspan="4" style="text-align:center; color:red;">Data tidak ditemukan</td></tr>');
                    }
                },
                error: function (xhr, status, error) {
        ...
        $('#searchInput').on('input', function () {
            ...
            loadVendors(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            var vendorId = $(this).data('id');  // Mendapatkan ID model dari atribut data-id
            var vendorName = $(this).closest('tr').find('td:nth-child(2)').text();  // Mendapatkan nama model dari atribut data-name

            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Vendor berikut : ' + vendorName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/vendor/delete/' + vendorId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Vendor sudah di delete, lihat data terbaru pada table.', 'success');
                                loadVendors(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');

                            } else {
                                Swal.fire('Error!', 'Gagal delete vendor.', 'error');
                            }
                        },
                        error: function (xhr, status, error) {
                            Swal.fire('Error!', 'Error ketika delete vendor ke Database.', 'error');
                        }
                    });
                }
            });
