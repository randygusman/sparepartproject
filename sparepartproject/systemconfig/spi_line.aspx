<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_line.aspx.cs" Inherits="sparepartproject.systemconfig.spi_line" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Line</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        ...
                        <li>
                            <a href="spi_line.aspx" style="background-color: white; color: #172679;">Line</a>
                        </li>
                        <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Line</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color:#f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color:#003366;">Line</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                    ...
                    <button id="btnAddModel" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                        <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Line
                    </button>
                </div>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 id="addModelModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Line</h5>
                            ...
                            <div class="modal-body">
                                <div style="padding-left: 20px; padding-right: 20px;">
                                    <div class="form-group mt-3 mb-5">
                                        <label for="modelName" style="font-weight: 500; font-size: 1rem;">New Line Name</label>
                                        <input type="text" class="form-control" id="lineName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" style="font-size: .8rem;" data-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Line</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        function loadLines(page, search = "") {
            $.ajax({
                url: '/api/line?page=' + page + '&pageSize=' + pageSize + '&search=' + encodeURIComponent(search),
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadLines(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
        ...
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadLines(currentPage); // Muat data untuk halaman 1 pada awalnya
        });
        ...
        $('#searchInput').on('input', function () {
            ...
            loadLines(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });

        $('#btnAddModelToDB').click(function () {
            var lineName = $('#lineName').val().toUpperCase(); // Ubah menjadi uppercase
            if (lineName) {
                // Menampilkan SweetAlert untuk konfirmasi
                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: "Line " + lineName + " akan ditambahkan.",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/api/line/check',  // URL endpoint untuk cek model
                            type: 'POST',
                            data: JSON.stringify({ lineName: lineName }),  // Kirim data model
                            contentType: 'application/json',
                            success: function (response) {
                                if (response.exists) {
                                    Swal.fire({
                                        title: "Double!",
                                        text: "Line tersebut sudah ada di Database?",
                                        icon: "warning"
                                    });
                                } else {
                                    // Jika tidak ada, simpan ke database
                                    $.ajax({
                                        url: '/api/line/add',  // URL endpoint untuk menambah model
                                        type: 'POST',
                                        data: JSON.stringify({ lineName: lineName }),
                                        contentType: 'application/json',
                                        success: function (result) {
                                            if (result.success) {
                                                Swal.fire('Berhasil', 'Line berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                                $('#addModelModal').modal('hide');  // Menutup modal
                                                // Reset nilai input
                                                $('#unitName').val('');
                                                $('#searchInput').val('');

                                                loadLines(currentPage);  // Muat ulang data model
                                            } else {
                ...
                notyf.error('New Line Name tidak boleh kosong!');
            }
        });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Line berikut : ' + unitName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/line/delete/' + unitId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Line sudah di delete, lihat data terbaru pada table.', 'success');
                                loadLines(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');
