<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_maker.aspx.cs" Inherits="sparepartproject.systemconfig.spi_maker" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Maker</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        ...
                        <li>
                            <a href="spi_maker.aspx" style="background-color: white; color: #172679;">Maker</a>
                        </li>
                        <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Maker</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color:#f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color:#003366;">Maker</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                    ...
                    <button id="btnAddModel" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                        <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Maker
                    </button>
                </div>
                <table class="table table-hover" id="makersTable" style="font-size: .8rem; cursor: pointer;">
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 id="addModelModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Maker</h5>
                            ...
                            <div class="modal-body">
                                <div style="padding-left: 20px; padding-right: 20px;">
                                    <div class="form-group mt-3 mb-4">
                                        <label for="vendorName" style="font-weight: 500; font-size: 1rem;">New Maker Name</label>
                                        <input type="text" class="form-control" id="makerName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                    </div>
                                    <div class="form-group mt-1 mb-5">
                                        <label for="suppName" style="font-weight: 500;">New Maker Code</label>
                                        <input type="text" class="form-control" id="makerCode" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" style="font-size: .8rem;" data-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Maker</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadMakers(currentPage); // Muat data untuk halaman 1 pada awalnya
        });
        ...
        $('#searchInput').on('input', function () {
            ...
            loadMakers(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });
        ...
        function loadMakers(page, search = "") {
            $.ajax({
                url: '/api/maker?page=' + page + '&pageSize=' + pageSize + '&searchTerm=' + encodeURIComponent(search), // Perbaikan di sini
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    $('#makersTable tbody').empty();

                    if (response.data && response.data.length > 0) {
                        // Menambahkan data ke dalam tabel
                        response.data.forEach(function (maker) {
                            $('#makersTable tbody').append(`
                        <tr>
                            <td>${maker.Id}</td>
                            <td>${maker.Nama_maker}</td>
                            <td>${maker.Kode_maker}</td>
                            <td>
                                <button class="btn btn-outline-danger btn-sm delete-model" data-id="${maker.Id}">
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadMakers(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
                        ...
                        $('#makersTable tbody').append('<tr><td colspan="4" style="text-align:center; color:red;">Data tidak ditemukan</td></tr>');
                    }
                },
        ...
        $('#btnAddModelToDB').click(function () {
            var makerRequest = {
                MakerName: $('#makerName').val().toUpperCase(),
                MakerKode: $('#makerCode').val().toUpperCase()
            };

            console.log(makerRequest);

            if (!makerRequest.MakerName || !makerRequest.MakerKode) {
                notyf.error('New Maker Name dan New Maker Code tidak boleh kosong!');
            ...
            Swal.fire({
                ...
                text: "Maker " + makerRequest.MakerName + " akan ditambahkan.",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/maker/check',
                        type: 'POST',
                        data: JSON.stringify(makerRequest), // Kirim data model
                        ...
                        success: function (response) {
                            if (response.exists) {
                                ...
                                Swal.fire({
                                    title: "Double!",
                                    text: "Maker tersebut sudah ada di database!",
                                    icon: "error"
                                });
                            } else {
                                ...
                                $.ajax({
                                    url: '/api/maker/add',
                                    type: 'POST',
                                    data: JSON.stringify(makerRequest),
                                    contentType: 'application/json',
                                    success: function (result) {
                                        if (result.success) {
                                            Swal.fire('Berhasil', 'Maker berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                            $('#addModelModal').modal('hide');
                                            $('#makerName').val('');
                                            $('#makerCode').val('');
                                            $('#searchInput').val('');

                                            loadMakers(currentPage);
                                        } else {
                                            Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan Maker.', 'error');
                                        }
                                    },
                        ...
                        error: function (error) {
                            notyf.error('Error saat memeriksa Maker Name yang sudah ada di Database!');
                        }
                    });
                }
            });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            var makerId = $(this).data('id');  // Mendapatkan ID model dari atribut data-id
            var makerName = $(this).closest('tr').find('td:nth-child(2)').text();  // Mendapatkan nama model dari atribut data-name

            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Maker berikut : ' + makerName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/maker/delete/' + makerId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Maker sudah di delete, lihat data terbaru pada table.', 'success');
                                loadMakers(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');

                            } else {
                                Swal.fire('Error!', 'Gagal delete Maker.', 'error');
                            }
                        },
                        error: function (xhr, status, error) {
                            Swal.fire('Error!', 'Error ketika delete Maker ke Database.', 'error');
                        }
                    });
                }
            });
