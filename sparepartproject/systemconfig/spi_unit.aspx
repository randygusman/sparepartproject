<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="spi_unit.aspx.cs" Inherits="sparepartproject.systemconfig.spi_unit" %>
...
<head runat="server">
    ...
    <title>Sparepart Inventory - System Config - Unit</title>
    <!-- Bootstrap CSS CDN -->
...
<body style="font-family: 'Poppins'; background-color: #ddd">
            ...
            <ul class="list-unstyled components" style="font-weight: 500;">
                <li class="active">
                    ...
                    <ul class="collapse list-unstyled" id="configSubmenu">
                        ...
                        <li>
                            <a href="spi_unit.aspx" style="background-color: white; color: #172679;">Unit</a>
                        </li>
                    </ul>
                </li>
                <li>
    ...
    <div id="content">
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            <div class="container-fluid">
                ...
                <h5 style="margin-bottom: 0px; margin-left: 20px;">System Config - Unit</h5>
                ...
                <div class="col-xl-12 col-lg-12">
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between bg-light" style="background-color:#f8f9fa!important">
                            <h6 class="m-0 font-weight-bold" style="color:#003366;">Unit</h6>
                        </div>
                        <!-- Card Body -->
                        <div class="card-body">
                    ...
                    <button id="btnAddModel" class="btn btn-primary" style="font-size: .7rem; display: inline-flex; align-items: center; padding: 8px 15px; border-radius: 5px; background-color: #007bff; border-color: #007bff;">
                        <i class="fas fa-plus" style="margin-right: 8px;"></i>Tambah Unit
                    </button>
                </div>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 id="addModelModalLabel" style="margin-bottom: 0; font-weight: 600; padding-left: 20px;">Tambah Unit</h5>
                            ...
                            <div class="modal-body">
                                <div style="padding-left: 20px; padding-right: 20px;">
                                    <div class="form-group mt-3 mb-5">
                                        <label for="modelName" style="font-weight: 500; font-size: 1rem;">New Unit Name</label>
                                        <input type="text" class="form-control" id="unitName" style="font-size: .8rem; width: 100%; text-transform: uppercase;" />
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" style="font-size: .8rem;" data-dismiss="modal">Close</button>
                                <button type="button" class="btn btn-primary" style="font-size: .8rem;" id="btnAddModelToDB">Save Unit</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        function loadUnits(page, search = "") {
            $.ajax({
                url: '/api/unit?page=' + page + '&pageSize=' + pageSize + '&search=' + encodeURIComponent(search),
                type: 'GET',
                dataType: 'json',
                success: function (response) {
                    ...
                    `);
                        ...
                        <li class="page-item ${activeClass}">
                            <a class="page-link" href="#" onclick="loadUnits(${i}, '${search}')">${i}</a>
                        </li>
                    `);
                        }
        ...
        $(document).ready(function () {
            ...
            setTitleHeader();
            loadUnits(currentPage); // Muat data untuk halaman 1 pada awalnya
        });
        ...
        $('#searchInput').on('input', function () {
            ...
            loadUnits(currentPage, searchTerm); // Panggil loadModels dengan kata kunci pencarian
        });

        $('#btnAddModelToDB').click(function () {
            var unitName = $('#unitName').val().toUpperCase(); // Ubah menjadi uppercase
            if (unitName) {
                // Menampilkan SweetAlert untuk konfirmasi
                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: "Unit " + unitName + " akan ditambahkan.",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        $.ajax({
                            url: '/api/unit/check',  // URL endpoint untuk cek model
                            type: 'POST',
                            data: JSON.stringify({ unitName: unitName }),  // Kirim data model
                            contentType: 'application/json',
                            success: function (response) {
                                if (response.exists) {
                                    Swal.fire({
                                        title: "Double!",
                                        text: "Unit tersebut sudah ada di Database?",
                                        icon: "warning"
                                    });
                                } else {
                                    // Jika tidak ada, simpan ke database
                                    $.ajax({
                                        url: '/api/unit/add',  // URL endpoint untuk menambah model
                                        type: 'POST',
                                        data: JSON.stringify({ unitName: unitName }),
                                        contentType: 'application/json',
                                        success: function (result) {
                                            if (result.success) {
                                                Swal.fire('Berhasil', 'Unit berhasil ditambahkan! Silahkan lihat pada table.', 'success');
                                                $('#addModelModal').modal('hide');  // Menutup modal
                                                // Reset nilai input
                                                $('#unitName').val('');
                                                $('#searchInput').val('');

                                                loadUnits(currentPage);  // Muat ulang data model
                                            } else {
                                                Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan Unit.', 'error');
                                            }
                                        },
                            ...
                            error: function (error) {
                                notyf.error('Error saat memeriksa Unit Name yang sudah ada di Database!');
                            }
                        });
                    }
                });
        ...
        $(document).on('click', '.delete-model', function (event) {
            ...
            var unitName = $(this).closest('tr').find('td:nth-child(2)').text();  // Mendapatkan nama model dari atribut data-name

            Swal.fire({
                title: 'Apakah Anda yakin?',
                text: 'Delete Unit berikut : ' + unitName + '?',  // Menampilkan nama model
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/api/unit/delete/' + unitId, // URL API delete
                        type: 'DELETE',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Deleted!', 'Unit sudah di delete, lihat data terbaru pada table.', 'success');
                                loadUnits(currentPage);  // Muat ulang data setelah dihapus
                                $('#searchInput').val('');
